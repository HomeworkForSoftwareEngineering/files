...
<?php
   $conn=mysqli_connect("localhost","root","11235813",'videodb');
    function createCommentRow($data){
        ...
        $sql = $conn->query("SELECT repvideo.id, comment, u_name, createdOn FROM repvideo WHERE commentID = '".$data['id']."' ORDER BY repvideo.id");
    ...
    if (isset($_POST['getAllComments'])) {
        ...
        $sql = $conn->query("SELECT commentsvideo.id, comment, u_name, createdOn FROM commentsvideo WHERE docId = '$dID' AND depName = '$dName' ORDER BY commentsvideo.id DESC LIMIT $start,20");
    ...
    if (isset($_POST['addComment'])) {
        ...
        if($isReply != 'false'){
            $conn->query("INSERT INTO repvideo (commentID, comment, createdOn, u_name) VALUES ('$commentID','$comment',NOW(), '$uID')");
            $sql = $conn->query("SELECT repvideo.id, comment, u_name, createdOn FROM repvideo ORDER BY repvideo.id DESC LIMIT 1");
        } else {
            $conn->query("INSERT INTO commentsvideo (u_name, comment, createdOn, depName, docId) VALUES ('$uID','$comment',NOW(), '$depName', '$docId')");
            $sql = $conn->query("SELECT commentsvideo.id, comment, u_name, createdOn FROM commentsvideo WHERE docId = '$docId' AND depName = '$depName' ORDER BY commentsvideo.id DESC LIMIT 1");
        }
    ...
    if (isset($_POST['send'])) {
        ...
        $sql = $conn -> query("SELECT rating FROM ratingsvideo WHERE u_id='$uID'");
        $Data = $sql->fetch_assoc();
        $s = $Data['rating'];
        
        if ($s == null){
            $conn->query("INSERT INTO ratingsvideo (rating,document_id,document_name,u_id) VALUES ('$ratedIndex','$dID','$dName','$uID')");
        } else{
            $sql = $conn -> query("SELECT document_id FROM ratingsvideo WHERE document_name ='$dName' AND u_id='$uID' AND document_id = '$dID'");
            $sData = $sql->fetch_assoc();
            $sID = $sData['document_id'];
            if($dID != $sID){
                $conn->query("INSERT INTO ratingsvideo (rating,document_id,document_name,u_id) VALUES ('$ratedIndex','$dID','$dName','$uID')");
            } else{
                $conn->query("UPDATE ratingsvideo SET rating='$ratedIndex' WHERE u_id='$uID' AND document_id='$dID' AND document_name='$dName'");
            }
        }
    ...
    $docId = $_GET['id'];
    $depName = $_GET['name'];
    $sqlNumofRep = $conn->query("SELECT repvideo.id FROM repvideo INNER JOIN commentsvideo ON repvideo.commentID = commentsvideo.id WHERE docId = '$docId' AND depName = '$depName'");
    $numReplies = $sqlNumofRep->num_rows;
    $sqlNumofCom = $conn->query("SELECT id FROM commentsvideo WHERE docId = '$docId' AND depName = '$depName'");
